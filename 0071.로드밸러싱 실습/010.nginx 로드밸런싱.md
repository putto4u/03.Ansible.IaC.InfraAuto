...
- name: 서버별 개별 식별 페이지 생성
  ansible.builtin.copy:
    ...
    mode: '0644'
  notify: Reload Nginx  # 파일 변경 시 핸들러 호출

# roles/webserver/handlers/main.yml에 위치
--- 
- name: Reload Nginx
...
# 핸들러 정의 (같은 파일 아래 혹은 roles/loadbalancer/handlers/main.yml에 위치)
- name: Restart Nginx
  ansible.builtin.service:
    name: nginx
    state: restarted
